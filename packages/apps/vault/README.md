...
No. Este paquete despliega el servidor de Vault directamente usando el chart oficial de HashiCorp; no depende de un operador separado.
Incluye el webhook Injector y el CSI driver opcional (ambos vienen en el chart original) para que las aplicaciones puedan inyectar secretos,
pero no instala ni requiere el **Vault Secrets Operator** u otros operadores de terceros. Si tu organización prefiere usar un operador para
gestionar secretos desde Kubernetes, puedes desplegarlo aparte y apuntarlo al mismo servidor de Vault provisto por este chart.
